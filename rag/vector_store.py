...
class VectorStoreService:
    ...
    def __init__(self):
        ...
        )  # 如果没有manifest_store关键字，则默认为 self.persist_directory/knowledge_manifest.json
    ...
    @staticmethod  # 静态方法装饰器。将一个函数绑定到类上，可以视为一个独立的函数，可以不需要实例化对象就可以调用。
    ...
    def _sync_manifest_from_legacy_md5(self, manifest: dict):
        ...
        try:
            with open(self.md5_hex_store, "r", encoding="utf-8") as f:
                legacy_md5s = {line.strip() for line in f if line.strip()}  # 返回一个集合
        except Exception as e:
        ...
        for path in allowed_files_path:
            ...
            if md5_hex in legacy_md5s:
                # 将每个文件的绝对路径转换为相对于 data_path 的相对路径，如 .../data/xxx.pdf -> xxx.pdf
    ...
    def reset_store(self, clear_md5=True):
        ...
        try:
            if os.path.exists(self.persist_directory):
                shutil.rmtree(self.persist_directory)  # 递归删除目录及其所有内容（包括子目录和文件）
        except Exception as e:
    ...
    def load_document(self, force_reload=False):
        ...
        self._cleanup_stale_documents(allowed_files_path)  # 删除已从 data/ 目录移除但在向量库中仍存在的旧文档
        manifest = self._sync_manifest_from_legacy_md5(self._load_manifest())
        ...
        for path in allowed_files_path:
            ...
            if not force_reload and manifest.get(relative_source, {}).get("md5") == md5_hex:
                ...
                continue
            try:
                documents = get_file_document(path)  # 每个 Document 包含 page_content 和 metadata
                if not documents:
                ...
                for document in documents:
                    ...
                    document.metadata["source_type"] = os.path.splitext(path)[1].lstrip(".").lower()  # 获取扩展名（如 ".txt"）、去掉前导点号、转为小写
                split_document = self._get_splitter(path).split_documents(documents)  # 根据文件类型（txt、pdf、其他）选择合适的切分器，将文档切分为小块
                if not split_document:
                    logger.warning(f"文件{path}没有被切分成任何文档，可能是内容过短或切分参数不合适")
                    continue
                self._delete_documents_by_source(relative_source)  # 删除向量库中该来源的所有旧文档，确保更新文件时不会残留旧版本切片
                for index, document in enumerate(split_document):
                    document.metadata["chunk_index"] = index  # 添加 chunk_index 序号元数据，用于后续的向量库查询
                # 为每个切片生成唯一且稳定的 ID，ID 格式：{source}:{chunk_index}:{content_hash}，如"故障排除.txt:0:a3f5b8c9d2e1"
                ids = [
